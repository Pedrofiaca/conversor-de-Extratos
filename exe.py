...
def upload_file():
    if request.method == 'POST':
        ...
        if file and file.filename.endswith('.pdf'):
            ...
            # Processa o PDF
            result_path = processar_pdf(file_path)
            filename = os.path.basename(result_path)
            return render_template('upload.html', filename=filename)  # Retorna a mesma página com o botão

        return render_template('upload.html', error="Arquivo inválido. Envie um arquivo PDF.")
